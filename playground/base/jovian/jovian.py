...
from torch.multiprocessing import Process, Pipe, Queue, Event
...
def _core_jovian_process(touch_radius, shared_cue_dict, direction, cnt, current_pos, current_hd, ball_vel, event_queue, start_routines_event):#,ready_event, start_gate):
    # soc_input, buf, buffer, buffering, emit, log, touch_radius, shared_cue_dict, rot,cnt, current_pos, current_hd, ball_vel
        ...
        try:
            ...
            while True:
                    tic = time()
                    if start_routines_event.is_set():
                        sync_routine()
                        read_routine()
                        task_routine()
                    else:
...
class Jovian(EventEmitter):
    ...
    def __init__(self):
        ...
        self._stop_decode_event = threading.Event()
        self.update_thread = None
        self.decode_thread = None

        self.start_routines_event = Event()
        self.bmi_set_tag=False
    ...
    def set_bmi(self, bmi, pos_buffer_len=30):
        ...
        self.bmi_pos_save=[]
        self.decoding_thread =  threading.Thread(target=self.decoding_loop, daemon=True)
        self.decoding_thread.start()
        self.bmi_set_tag=True

        

        @self.bmi.binner.connect
        def on_decode(X):
            # self.log.info(f'send X!')
            q_size = self.decode_queue.qsize()
            self.decode_queue.put_nowait(X)
            self.log.info('decode queue sending {}'.format(q_size))
    ...
    def decoding_loop(self):
        ...
        with Timer('decoding', verbose=False): # use verbose to see the decoding time, rouphly 16 ms for 500 units
            ...
            while not self._stop_decode_event.is_set():
                tic = time()
                try:
                    
                    X = self.decode_queue.get(block=True, timeout=0.05)
                    if X is None:
                        break
                    if hasattr(self.bmi.dec, 'model'):
                    ...
                    self.log.warn(f'Decoding loop error: {e}')
                self.log.info('decoding_loop time is {}'.format(time()-tic))
    ...
    def bmi_close(self):
        self._stop_decode_event.set()
        if self.decoding_thread:
    ...
    def start(self):
        ...
        self.jovian_process = Process(target=_core_jovian_process,args=(self.touch_radius, self.shared_cue_dict, self.rot.direction, self.cnt, self.current_pos, self.current_hd, self.ball_vel,self.event_queue, self.start_routines_event,),name='jovian')
        self.jovian_process.daemon = True
        ...
        if not self.bmi_set_tag:
            self.start_routines_event.set()
        self.jovian_process.start()

    def bmi_start(self):
        """Call this function to unlock sync_routine and task_routine"""
        self.log.info("BMI Start: Unlocking sync and task routines.")
        self.start_routines_event.set()
