...
def _core_jovian_process(touch_radius, shared_cue_dict, direction, cnt, current_pos, current_hd, ball_vel, event_queue):#,ready_event, start_gate):
    # soc_input, buf, buffer, buffering, emit, log, touch_radius, shared_cue_dict, rot,cnt, current_pos, current_hd, ball_vel
        ...
        def sync_routine():
            nonlocal flag, sync_count,syncer,start_loop,end_pulse
            data=[0]
            if syncer != 'Jovian':
            ...
            if syncer == 'Jovian':
                if time()-start_loop>SEQUENCE_S:
                    start_loop=time()
                if time()-start_loop>=TRIGGER_S[sync_count]:                 
                    data=[1]
                    End_pulse=TRIGGER_S[sync_count]+PULSE_S
                if end_pulse:
...
def file_saving_loop(saving_queue):
    
    # Check for events sent from the subprocess
    
    try:
        while True:
            data = saving_queue.get(block=True, timeout=0.05)
            # self.log.info('we are saving scv as {}'.format(data))
            if data.get('scv') is not None:
                with open('./scv.bin', 'ab+') as f_scv:
                   f_scv.write(data['scv'].tobytes())
    except Exception as e:
            print(f"Subprocess error: {e}")
    finally:
        print("closing file_saving_loop")
...
class Jovian(EventEmitter):
    ...
    def set_bmi(self, bmi, pos_buffer_len=30):
        ...
        self.bmi_pos_save=[]
        self.saving_process=Process(target=file_saving_loop,args=(self.saving_queue, ),name='bmi_saving')
        self.saving_process.daemon = True
        ...
        def on_decode(X):
            ...
            with Timer('decoding', verbose=False): # use verbose to see the decoding time, rouphly 16 ms for 500 units
                ...
                # self.saving_queue.put_nowait({'scv':X})
                
                if not len(self.scv_save):
                    self.scv_save = X
                elif len(self.scv_save)>200:
                    self.saving_queue.put_nowait({'scv':self.scv_save})
                    self.scv_save=[]
                else:
                    self.scv_save = np.vstack((self.scv_save,X))
    ...
    def bmi_close(self):
        ...
        if len(self.scv_save):
            f_scv.write(self.scv_save.tobytes())
            self.scv_save = []
