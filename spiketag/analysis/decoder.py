...
def load_model_and_config(model_path, config_path):    
    ...
    dec = DeepOSC(
        ...
        bn=config['model_config']['bn'],
        LSTM=config['model_config']['LSTM'],
        t_window=config['model_config']['t_window'],
        t_step=config['model_config']['t_step']
    )
    
    # restore the position mean
    dec.pos_mean_original = np.array(config['pos_mean_original'])

    # Load weights
    dec.model.load_state_dict(torch.load(model_path))

    # Load neuron indices
    neuron_idx = np.array(config['neuron_config']['neuron_idx'])
    
    return dec, neuron_idx
...
#         x = torch.sin(x)
        ...
        x = F.softmax(F.relu(self.fc1(x)), dim=1) * x
...
class DeepOSC(Decoder):
    ...
    def __init__(self, input_dim, hidden_dim=[128, 128], output_dim=2, bn=False, LSTM=False, t_window=2, t_step=0.1):
       ...
       # Tong added, 05.28.2025
        self.max_running_data = int(3600/t_step) # accumulate 60 min data for batch norm
    ...
    def predict(self, X, cuda=True, mode='eval', bn_momentum=0.1, target='pos'):
        '''
        X: a numpy array of firing rates of neurons 
        (T_step, B_bins, N_neurons) or (T_step, N_neurons*B_bins)
        '''
        if X.ndim == 3:
            X = X[:,:,self.neuron_idx].reshape(X.shape[0], -1)
        if cuda:
            self.model.cuda()
    ...
    def update_bn(self, cuda=True, bn_momentum=0.9):
        ...
        # Tong added, 05.28.2025
    def _update_running_data(self, new_data):
        """Update running data using FIFO queue approach"""
        if len(self._running_data) >= self.max_running_data:
            self._running_data.pop(0)  # Remove the oldest sample (first in)
        self._running_data.append(new_data)  # Add new sample (last in)
    def reset_bn(self):
        self.model.bn1.track_running_stats = True
        self.model.bn1.running_mean = torch.zeros((256,)).float().cuda()
        self.model.bn1.running_var = torch.ones((256,)).float().cuda()  # Initialize to ones, not zeros   
    def predict_rt(self, X, cuda=True, mode='eval', bn_momentum=0.1):
        ...
        self._update_running_data(X[..., self.neuron_idx].ravel())
    ...
    def save_model_and_config(self, folder='decoder', model_path='model_weights.pt', config_path='model_config.yaml'):
        os.makedirs(folder, exist_ok=True)
        # Save model weights
        torch.save(self.model.state_dict(), os.path.join(folder, model_path))
        ...
        config = {
            ...
            'pos_mean_original': self.pos_mean_original.tolist(),
            'neuron_config': {
                'n_neurons': len(self.neuron_idx),
                'neuron_idx': self.neuron_idx.tolist(),  # Convert numpy array to list for YAML serialization
            }
        }
        
        with open(os.path.join(folder, config_path), 'w') as f:
            yaml.dump(config, f)
