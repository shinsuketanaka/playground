...
from torch.multiprocessing import Process, Pipe, SimpleQueue, set_start_method, Queue, Event
...
class SpikeStreamClient:
    ...
    def _receive_loop(self):
        ...
        while self.running:
            ...
            except (OSError, ConnectionResetError, AttributeError):
                # If the socket is closed by the main thread, 
                # or the connection is reset, exit the loop silently.
                if not self.running:
                    break 
...
def BMI_core_stream(spike_data_queue,stop_decoding):
    def process_spikes(time_stamps, unique_ids, probe_index):
        """put spike data into spike_data_queue for binner and decoding"""
        # global spike_data_queue, cluster_offsets, probe_indices
        
        # Apply the correct cluster ID offset based on the probe index
        try:
            matches = np.where(probe_indices == probe_index)[0]
            offset = int(cluster_offsets[matches[0]]) if len(matches) else 0
        except Exception as e:
            print(f"Warning: failed to map offset for probe_index={probe_index}: {e}. Using offset 0.")
            offset = 0
        unique_ids_with_offset = unique_ids + offset
        #print(unique_ids_with_offset)
        # print('process_spikes:{}'.format(time_stamps))
        try:
            # print("try to send spike_data_queue")
            spike_data_queue.put_nowait({
                'time_stamps': time_stamps, 
                'unique_ids': unique_ids_with_offset
            })
            q_size = spike_data_queue.qsize()
            if q_size > 100:
                print(f"[Warning] Queue is backing up! Current size: {q_size}")
        except Exception as e:
            print(f"Queue Put Error: {e}")

    # --- Load Probe Configuration ---
    probe_params_path = './decoder/probe_params.npz'
    if not os.path.exists(probe_params_path):
        print(f"ERROR: Probe parameters file not found at '{probe_params_path}'")
        print("Please run the full data preparation pipeline (including step 2) first.")
        exit()
    
    probe_params = np.load(probe_params_path)
    probe_count = probe_params['probe_count'].item()
    cluster_offsets = probe_params['cluster_offsets']
    probe_indices = probe_params.get('probe_indices', np.arange(probe_count, dtype=np.int32))
    
    print(f"Loaded configuration for {probe_count} probes.")
    print(f"  - Cluster Offsets: {cluster_offsets}")
    ...
    for i in range(probe_count):
        port = base_port + i
        client = SpikeStreamClient(host='localhost', port=port, callback=process_spikes)
        clients.append(client)
    print("Trying to connect to spike servers...")
    
    all_connected = False
    
    while not all_connected:
        # Attempt to connect all clients. The client.connect() method will print its own status.
        connection_statuses = [client.connect() for client in clients]
        
        if all(connection_statuses):
            all_connected = True
            print("All servers connected.")
        else:
            print("Failed to connect to one or more servers. Disconnecting and retrying in 1 second...")
            # Disconnect any that were successful to start fresh
            for i, status in enumerate(connection_statuses):
                if status:
                    clients[i].disconnect()
            time.sleep(1)

        # If all connected, start receiving data
    if all_connected:
        for client in clients:
            client.start_receiving()
        print("All clients receiving data. Press Ctrl+C to stop.")
...
class BMI(object):
    ...
    def __init__(self, prb=None,spike_df_file=None, ttlport=None):
        ...
        if spike_df_file is not None:
            import pandas as pd
            spike_df = pd.read_pickle(spike_df_file)
            unit_ids = sorted(spike_df['spike_id'].unique())
            self.n_neurons = max(unit_ids)+1
        ...
        self.spike_data_queue = None
        self._stop_event = threading.Event()
        self.update_thread = None
        self.stop_decoding = Event()
    ...
    def set_binner(self, bin_size, B_bins):
        ...
        '''
        N_units = self.n_neurons # The unit #0, no matter from which group, is always noise
        self.binner = Binner(bin_size, N_units, B_bins)    # binner initialization (space and time)      
        # self.binner.log=self.log
        self.log.info('BMI binner: {} bins {} units, each bin is {} seconds'.format(B_bins, N_units, bin_size))  
        self.log.info('---2. BMI binner initiation succeed---\n')
        # @self.binner.connect
        # def on_decode(X):
    ...
    def set_decoder(self, dec, dec_file=None):
        ...
        self.log.info('---3. BMI Decoder initiation succeed---\n')
    ...
    def stream_to_binner(self,gui_queue,model=None):
        # global spike_data_queue
        self.model=model
        # self.log.info('stream_to_binner just started!')
        while not self._stop_event.is_set():
            # self.log.info('stream_to_binner is running!')
           
            with Timer('real-time decoding', verbose=False):
                try:
                    
                    bmi_output = self.spike_data_queue.get(block=True, timeout=0.05)
                    
                    if any(bmi_output):
                        if self.binner is not None:
                            self.binner.input(bmi_output) 
                            # self.log.info('bmi_output: {}'.format(bmi_output))
            
                        # if self.gui_queue is not None:
                        #     self.gui_queue.put(bmi_output)

                    # self.spike_data_queue.task_done()
                except Empty: # from queue import Empty
                # except  self.spike_data_queue.empty():
                    continue 
                except Exception as e:
                    self.log.info(f"Error in stream_to_binner: {e}")
               

        self.log.info('stopped stream_to_binner')
    ...
    def start(self, gui_queue=False, model=None):
        ...
        self.spkGLX_process = Process(target=BMI_core_stream, args=(self.spike_data_queue,self.stop_decoding,), name='spkGLX')
        
        self.spkGLX_process.daemon = True

        self.update_thread = threading.Thread(target=self.stream_to_binner, args=(self.gui_queue, model),daemon=True)
        self.update_thread.start()

        
        self.spkGLX_process.start()  
    ...
    def stop(self):
        ...
        self.spkGLX_process.join(timeout=2.0)
        if self.spkGLX_process.is_alive():
            self.spkGLX_process.terminate()
            self.spkGLX_process.join()
        self.spike_data_queue = None
